...
#define usCiclo 6000   // Cicle d'execució del software en microsegons
...
#define pin_INT_Yaw 9       // Pin Yaw del mando RC
#define pin_INT_Pitch 12    // Pin Pitch del mando RC
#define pin_INT_Roll 7      // Pin Roll del mando RC
#define pin_LED_vermell1 10    // Pin LED vermell 1      
#define pin_LED_vermell2 13    // Pin LED vermell 2    
#define pin_LED_blau 11     // Pin LED blau    
#define pin_LED_groc A2  // Pin LED groc    
// --------------------------------------------------------------------------------
...
const int us_max_Throttle_adj = 1500; // <-- Canviar per a ajustar la potencia global dels motors
const int us_min_Throttle_adj = 950; // <-- Canviar per a ajustar la potencia global dels motors
const float us_max_Throttle_raw = 2000;  // <-- Si la entrada Throttle está invertida sustituir este valor
const float us_min_Throttle_raw = 992;  // <-- por este y viceversa

// AJUSTE MANDO RC - PITCH
const float us_max_Pitch_raw = 1996;
const float us_min_Pitch_raw = 968;
...
const float us_max_Roll_raw = 1984;
const float us_min_Roll_raw = 948;
...
// AJUSTE MANDO RC - YAW
const float us_max_Yaw_raw = 2000;
const float us_min_Yaw_raw = 980;
...
void setup() {
  ...
  // Declaración de LEDs
  pinMode(pin_LED_groc, OUTPUT); // Led naranja --> Batería baja
  pinMode(pin_LED_blau, OUTPUT);    // Led azul    --> Ciclo (parpadeo)
  pinMode(pin_LED_vermell1, OUTPUT);   // Led rojo 1  --> Error MPU6050
  pinMode(pin_LED_vermell2, OUTPUT);   // Led rojo 2  --> Tiempo de ciclo o de 1ms excedido
  ...
  digitalWrite(pin_LED_vermell2, HIGH);
  Serial.println("Esperant");
  while (RC_Throttle_consigna < 1000 || RC_Throttle_consigna > 1025) RC_procesar();
  Serial.println("Acabat");
  digitalWrite(pin_LED_vermell2, LOW);
  ...
  digitalWrite(pin_LED_vermell1, HIGH);
  delay(500);
  digitalWrite(pin_LED_vermell1, LOW);
...
void loop() {
  ...
  if (micros() - loop_timer > usCiclo + 50){ digitalWrite(pin_LED_vermell2, HIGH);} 
...
void RC_procesar() {
  ...
  RC_Roll_consigna     = map(RC_Roll_filt, us_min_Roll_raw, us_max_Roll_raw, us_min_Roll_adj, us_max_Roll_adj) - 1;
...
void Modulador() {
  ...
  if (RC_Throttle_consigna <= min_Activar_PID) {
    PID_W_Pitch_I = 0;
    PID_W_Roll_I = 0;
    PID_W_Yaw_I  = 0;
    PID_ang_Pitch_I = 0;
    ...
    if (ESC1_us < 1000) ESC1_us = 900;
    if (ESC2_us < 1000) ESC2_us = 900;
    if (ESC3_us < 1000) ESC3_us = 900;
    if (ESC4_us < 1000) ESC4_us = 900;
  }

  // Si el throttle es mayor a 1300us, el control de estabilidad se activa.
  else {
    // Limitar throttle a 1800 para dejar margen a los PID
    if (RC_Throttle_consigna > 1500)RC_Throttle_consigna = 1500;
    ...
    if (ESC1_us < 1050) ESC1_us = 1050;
    if (ESC2_us < 1050) ESC2_us = 1050;
    if (ESC3_us < 1050) ESC3_us = 1050;
    if (ESC4_us < 1050) ESC4_us = 1050;
    // Evitamos mandar consignas mayores a 2000us a los motores
    if (ESC1_us > 1600) ESC1_us = 1600;
    if (ESC2_us > 1600) ESC2_us = 1600;
    if (ESC3_us > 1600) ESC3_us = 1600;
    if (ESC4_us > 1600) ESC4_us = 1600;
  }
}
...
void PWM() {
  ...
  if (tiempo_ON > 900) digitalWrite(pin_LED_vermell2, HIGH);
  // ------------------ ¡¡1ms max!! ------------------
...
void Lectura_tension_bateria() {
  ...
  if (RC_Throttle_consigna < 1100) {
    ...
    tension_bateria = 2.5 * (lectura_ADC * 4.44  / 1023);
...
void LED_blink() {
  if (LED_contador % 20 == 0) {
    if (digitalRead(pin_LED_blau) == LOW) digitalWrite(pin_LED_blau, HIGH);
    else digitalWrite(pin_LED_blau, LOW);

    if (digitalRead(pin_LED_blau) == HIGH && LOW_BAT_WARING == 1){analogWrite(pin_LED_groc, 0);}
    else {analogWrite(pin_LED_groc, 255);}
  }
  LED_contador++;
}
